...
HRESULT CLoopbackCapture::OnAudioSampleRequested()
{
	...
	while (SUCCEEDED(m_AudioCaptureClient->GetNextPacketSize(&FramesAvailable)) && FramesAvailable > 0)
	{
		...
		// Write every packet to stdout — including silent ones — so the consumer
		// gets a sample-rate-accurate stream with no gaps. Drop the silence
		// optimization (upstream had it) and flush after each write to keep
		// pipe latency under one packet.
		if (m_DeviceState != DeviceState::Stopping)
		{
